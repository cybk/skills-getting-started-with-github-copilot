document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = details.participants.length
          ? details.participants
              .map(
                (participant) =>
                  `<li class="participant-item"><span class="participant-email">${participant}</span><button type="button" class="participant-remove-btn" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(participant)}" title="Unregister participant" aria-label="Unregister ${participant}"><span aria-hidden="true" class="participant-remove-icon">🗑</span></button></li>`
              )
              .join("")
  ...
  // Handle participant unregister button click using event delegation
